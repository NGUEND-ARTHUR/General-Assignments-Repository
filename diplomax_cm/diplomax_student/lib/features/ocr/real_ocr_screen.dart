...
class OcrService {
  ...
  Future<OcrResult> extractFromFile(File imageFile) async {
    ...
    try {
      ...
      return OcrResult(fullText: fullText, fields: fields, blocks: recognized.blocks);
    } finally {
      recognizer.close();
    }
  }
  ...
  Map<String, String> _parseAcademicFields(
    ...
    // Year / Date
    final yearMatch = RegExp(r'(?:année|year|class of|promotion)\s*:?\s*(\d{4})',
        caseSensitive: false).firstMatch(text);
    ...
    final avgMatch = RegExp(
        ...
        caseSensitive: false).firstMatch(text);
    if (avgMatch != null) fields['average'] = avgMatch.group(1)!.trim().replaceAll(',', '.');
...
class OcrResult {
  ...
  OcrResult({required this.fullText, required this.fields, required this.blocks});
}
...
class _RealOcrState extends ConsumerState<RealOcrScreen> {
  ...
  Future<void> _pickAndProcess(ImageSource source) async {
    try {
      ...
      setState(() { _imageFile = file; _processing = true; _result = null; _error = null; });
        ...
        children: [
          ...
          Row(children: [
            Expanded(child: ElevatedButton.icon(
              ...
              onPressed: _processing ? null : () => _pickAndProcess(ImageSource.camera))),
            const SizedBox(width: 10),
            Expanded(child: OutlinedButton.icon(
              ...
              onPressed: _processing ? null : () => _pickAndProcess(ImageSource.gallery))),
          ]),
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text('Extracted fields (${fields.length})',
  ...
  Widget _rawTextCard() => ExpansionTile(
    title: Text('Full extracted text',
      style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w500)),
    children: [
      Container(
  ...
  void _saveToVault() {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text('Document data saved to vault', style: GoogleFonts.dmSans()),
