...
class OcrService {
  ...
  Map<String, String> _extractFields(String text, List<TextBlock> blocks) {
    ...
    if (matMatch != null) result['matricule'] = matMatch.group(0)!.toUpperCase();
    ...
    if (nameMatch != null) result['full_name'] = _cleanName(nameMatch.group(1)!);
    ...
    final years = yearPattern.allMatches(text).map((m) => m.group(0)!).toSet().toList();
    ...
    } else if (text.toLowerCase().contains('relevé de notes') || text.toLowerCase().contains('transcript')) {
      result['document_type'] = 'transcript';
    } else if (text.toLowerCase().contains('attestation')) {
      result['document_type'] = 'attestation';
    } else if (text.toLowerCase().contains('certificat') || text.toLowerCase().contains('certificate')) {
    ...
    if (degreeMatch != null) result['degree'] = degreeMatch.group(0)!.toLowerCase();
  ...
  String _cleanName(String raw) =>
      raw.trim().split(' ').map((w) => w.isEmpty ? '' :
...
class OcrScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<OcrScreen> createState() => _OcrScreenState();
}
...
class _OcrScreenState extends ConsumerState<OcrScreen> {
  ...
  Future<void> _pickAndProcess(ImageSource source) async {
    ...
    setState(() { _image = file; _phase = _OcrPhase.processing; _error = null; });

    try {
      final result = await _svc.recognizeText(file);
      setState(() { _result = result; _phase = _OcrPhase.done; });
    } catch (e) {
      setState(() { _phase = _OcrPhase.error; _error = e.toString(); });
    }
  }

  void _reset() => setState(() {
    _phase = _OcrPhase.idle; _image = null; _result = null; _error = null;
  });
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      ...
      leading: BackButton(onPressed: () => context.go('/home'), color: _textPri),
      title: Text('Scan document (OCR)',
    ...
    body: SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        _imagePreview(),
        const SizedBox(height: 20),
        if (_phase == _OcrPhase.idle || _phase == _OcrPhase.error) _pickButtons(),
  ...
  Widget _infoBox() => Container(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Text(
        ...
        style: GoogleFonts.dmSans(fontSize: 12, color: _amber, height: 1.5))),
    ]));
    ...
    child: ClipRRect(
      ...
      child: _image != null
          ...
          : Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.document_scanner_rounded, color: Colors.white30, size: 48),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Extracted fields', style: GoogleFonts.dmSans(
          ...
          Container(
            ...
            child: Text('${f.length} fields · ${(_result!.confidence * 100).toInt()}% confidence',
              style: GoogleFonts.dmSans(fontSize: 10, color: _green, fontWeight: FontWeight.w500))),
        ]),
        const SizedBox(height: 12),
        ...
        if (f.isEmpty)
          Padding(
            ...
            child: Text(
              ...
              style: GoogleFonts.dmSans(fontSize: 12, color: _textSec, height: 1.5)))
        else
        ...
        if (_result!.rawText.isNotEmpty) ...[
          ...
          const Divider(height: 1, color: Color(0xFFE0DDD5)),
          const SizedBox(height: 10),
          Text('Raw extracted text', style: GoogleFonts.dmSans(
            fontSize: 12, color: _textSec, fontWeight: FontWeight.w500)),
          const SizedBox(height: 6),
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
              color: const Color(0xFFF1EFE8), borderRadius: BorderRadius.circular(8)),
            child: Text(
              _result!.rawText.length > 400
                  ? '${_result!.rawText.substring(0, 400)}...'
                  : _result!.rawText,
              style: GoogleFonts.dmSans(fontSize: 10, color: _textSec, height: 1.5))),
        ],
      ]));
  }
