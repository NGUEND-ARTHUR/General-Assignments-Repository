  ...
  routes: [
    ...
    GoRoute(
      ...
      routes: [
        ...
        GoRoute(path: 'document/:id', builder: (_, s) => DocumentDetailScreen(documentId: s.pathParameters['id']!)),
        GoRoute(path: 'search',       builder: (_, __) => const DocumentSearchScreen()),
        GoRoute(path: 'qr-generate',  builder: (_, __) => const QrGenerateScreen()),
        ...
        GoRoute(path: 'share/:documentId', builder: (_, s) => ShareDocumentScreen(
        ...
        GoRoute(path: 'payment/:product', builder: (_, s) => PaymentScreen(initialProduct: s.pathParameters['product'])),
        GoRoute(path: 'profile',          builder: (_, __) => const ProfileScreen()),
        GoRoute(path: 'international-share', builder: (_, __) => const InternationalShareScreen(documentIds: [], documentTitles: [])),
        GoRoute(path: 'request',          builder: (_, __) => const DocumentRequestScreen()),
      ],
    ),
  ],
);
...
void main() async {
  ...
  runApp(const ProviderScope(child: DiplomaxStudentApp()));
}

class DiplomaxStudentApp extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => MaterialApp.router(
    ...
    theme: ThemeData(
      useMaterial3: true,
      colorScheme: ColorScheme.fromSeed(seedColor: _green, primary: _green, surface: _surface, background: _bg),
      textTheme: GoogleFonts.dmSansTextTheme(),
      appBarTheme: const AppBarTheme(backgroundColor: Colors.transparent, elevation: 0, scrolledUnderElevation: 0, iconTheme: IconThemeData(color: Color(0xFF1A1A1A))),
...
class _SplashScreen extends StatefulWidget {
  const _SplashScreen();
  @override State<_SplashScreen> createState() => _SS();
}
class _SS extends State<_SplashScreen> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: FadeTransition(opacity: _f, child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width: 90, height: 90, decoration: BoxDecoration(color: Colors.white.withOpacity(0.15), borderRadius: BorderRadius.circular(24)), child: const Icon(Icons.school_rounded, color: Colors.white, size: 50)),
      const SizedBox(height: 24),
      Text('Diplomax CM', style: GoogleFonts.instrumentSerif(fontSize: 36, color: Colors.white)),
      const SizedBox(height: 8),
      Text('Secure Academic Credentials', style: GoogleFonts.dmSans(fontSize: 13, color: Colors.white70, fontWeight: FontWeight.w300)),
    ]))),
  );
}
