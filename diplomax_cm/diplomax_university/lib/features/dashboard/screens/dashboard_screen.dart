...
class _DashState extends ConsumerState<DashboardScreen> {
  ...
  Widget _buildHeader() => Container(
    ...
    child: Row(children: [
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Dashboard', style: GoogleFonts.instrumentSerif(fontSize: 22, color: _textPri)),
        Text('The ICT University — Yaoundé',
          style: GoogleFonts.dmSans(fontSize: 12, color: _textSec, fontWeight: FontWeight.w300)),
      ]),
      const Spacer(),
      ElevatedButton.icon(
  ...
  Widget _statsGrid(bool isWide) {
    final d = _data ?? {};
    final stats = [
      ('${d['total_students']  ?? 0}', 'Students', Icons.people_rounded,    _green,  _greenLight),
      ('${d['total_documents'] ?? 0}', 'Documents issued', Icons.description_rounded, _blue, _blueLight),
      ('${d['blockchain_anchored'] ?? 0}', 'On blockchain', Icons.verified_rounded, _green, _greenLight),
    ];
    return GridView.count(
      ...
      children: stats.map((s) => Container(
        ...
        decoration: BoxDecoration(
          color: s.$5, borderRadius: BorderRadius.circular(14),
          border: Border.all(color: s.$4.withOpacity(0.3))),
        child: Row(children: [
          Icon(s.$3, color: s.$4, size: 28),
          const SizedBox(width: 14),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(s.$1, style: GoogleFonts.instrumentSerif(fontSize: 26, color: s.$4)),
            Text(s.$2, style: GoogleFonts.dmSans(fontSize: 11, color: s.$4.withOpacity(0.8))),
          ]),
        ]),
      )).toList(),
    );
  ...
  Widget _blockchainStatus() => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(
        color: (_blockchainHealthy ? _green : _amber).withOpacity(0.3))),
    child: Row(children: [
      Icon(
        _blockchainHealthy ? Icons.link_rounded : Icons.link_off_rounded,
      ...
      TextButton(
        onPressed: _load,
        child: Text('Refresh', style: GoogleFonts.dmSans(
          fontSize: 12, color: _blockchainHealthy ? _green : _amber))),
    ]),
  );
  ...
  Widget _quickActions(BuildContext ctx) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('Quick actions', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w500)),
      const SizedBox(height: 12),
      Row(children: [
        _actionCard(ctx, Icons.add_circle_rounded, 'Issue diploma', _green,   '/issue'),
        const SizedBox(width: 10),
        _actionCard(ctx, Icons.people_rounded,     'Students',      _blue,    '/students'),
        const SizedBox(width: 10),
        _actionCard(ctx, Icons.folder_rounded,     'Documents',     _amber,   '/documents'),
      ]),
    ],
  );

  Widget _actionCard(BuildContext ctx, IconData icon, String label, Color c, String route) =>
  ...
  Widget _recentDocuments() {
    ...
    return Column(
      ...
      children: [
        Row(children: [
          Text('Recent documents', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w500)),
          const Spacer(),
          TextButton(
            onPressed: () => context.go('/documents'),
            child: Text('View all', style: GoogleFonts.dmSans(color: _green, fontSize: 12))),
        ]),
        const SizedBox(height: 10),
        if (docs.isEmpty)
          Center(child: Text('No documents issued yet.',
          ...
          ...docs.take(8).map((d) => Container(
            ...
            child: Row(children: [
              Container(
                ...
                child: const Icon(Icons.description_rounded, color: _green, size: 18)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(d['title'] as String? ?? '', style: GoogleFonts.dmSans(
                  fontSize: 13, fontWeight: FontWeight.w500)),
