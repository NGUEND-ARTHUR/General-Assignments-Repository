...
class DocumentTemplate {
  ...
  const DocumentTemplate({
    ...
    required this.docType, required this.degree, required this.field,
    required this.titleTemplate, required this.preFilledCourses,
...
const _templates = [
  DocumentTemplate(
    ...
    docType: 'diploma', degree: 'Licence (Bachelor)',
    ...
    icon: Icons.computer_rounded, color: _green, bgColor: _greenLight,
  ),
  DocumentTemplate(
    id: 'transcript_standard',
    name: 'Official Transcript',
    description: 'Standard academic transcript for all programmes',
    docType: 'transcript', degree: 'Licence (Bachelor)',
    field: '', titleTemplate: 'Relevé de Notes Officiel',
    preFilledCourses: [],
    icon: Icons.description_rounded, color: _blue, bgColor: _blueLight,
  ),
  DocumentTemplate(
    id: 'attestation_inscription',
    name: 'Attestation d\'inscription',
    description: 'Certificate confirming student enrolment',
    docType: 'attestation', degree: '',
    field: '', titleTemplate: 'Attestation d\'Inscription',
    preFilledCourses: [],
    icon: Icons.assignment_rounded, color: _coral, bgColor: _coralLight,
  ),
  DocumentTemplate(
  ...
  DocumentTemplate(
    ...
    titleTemplate: 'Licence en Science des Données et Intelligence Artificielle',
    preFilledCourses: [
...
class TemplateFillScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<TemplateFillScreen> createState() => _TemplateState();
}
...
class _TemplateState extends ConsumerState<TemplateFillScreen> {
  ...
  void _selectTemplate(DocumentTemplate tpl) {
    ...
    _gradeCtrl.clear(); _creditCtrl.clear();
    ...
    setState(() { _selected = tpl; _phase = _TplPhase.fillFields; });
  }
  ...
  Future<void> _issue() async {
    if (_matCtrl.text.trim().isEmpty) {
      setState(() => _error = 'Student matricule is required');
      return;
    }
    ...
    setState(() { _phase = _TplPhase.issuing; _error = null; });
    ...
    try {
      final dio = Dio(BaseOptions(baseUrl: const String.fromEnvironment(
          'API_BASE_URL', defaultValue: 'https://api.diplomax.cm/v1')));
      ...
      setState(() {
        _error = (e.response?.data as Map?)?['detail']?.toString() ?? 'Issuance failed';
  ...
  Widget _buildBody() {
    switch (_phase) {
      case _TplPhase.selectTemplate: return _selectView();
      case _TplPhase.fillFields:     return _fillView();
      case _TplPhase.issuing:
        return const Center(child: CircularProgressIndicator(color: _green));
      case _TplPhase.done: return _doneView();
    }
  }

  Widget _selectView() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('Choose a template to pre-fill all standard fields.',
        style: GoogleFonts.dmSans(fontSize: 13, color: _textSec,
          fontWeight: FontWeight.w300)),
      const SizedBox(height: 16),
      Expanded(child: ListView(children: _templates.map((t) =>
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                Text(t.name,style:GoogleFonts.dmSans(
                  fontSize:13,fontWeight:FontWeight.w500)),
                Text(t.description,style:GoogleFonts.dmSans(
                  fontSize:11,color:_textSec,fontWeight:FontWeight.w300)),
                if(t.preFilledCourses.isNotEmpty) ...[
                  const SizedBox(height:4),
                  Text('${t.preFilledCourses.length} courses pre-loaded',
                    style:GoogleFonts.dmSans(fontSize:10,color:t.color)),
                ],
              ])),
    ...
    return SingleChildScrollView(child: Column(
      ...
      children: [
        ...
        Container(padding:const EdgeInsets.all(12),
          decoration:BoxDecoration(color:tpl.bgColor,borderRadius:BorderRadius.circular(10),
            border:Border.all(color:tpl.color.withOpacity(0.2))),
          child:Row(children:[
            Icon(tpl.icon,color:tpl.color,size:18),
            const SizedBox(width:10),
            Expanded(child:Text(tpl.name,
              style:GoogleFonts.dmSans(fontSize:13,fontWeight:FontWeight.w500,color:tpl.color))),
            GestureDetector(
              onTap:()=>setState(()=>_phase=_TplPhase.selectTemplate),
              child:Text('Change',style:GoogleFonts.dmSans(color:tpl.color,fontSize:12))),
          ])),
        const SizedBox(height:16),

        // Student-specific fields
        _sectionHeader('Student information'),
        _tf(_matCtrl,'Matricule *','ICTU20223180',Icons.badge_rounded),
        const SizedBox(height:10),
        _tf(_dateCtrl,'Issue date *','Tap to pick',Icons.calendar_today_rounded,
        ...
        _sectionHeader('Document details (pre-filled, editable)'),
        _tf(_titleCtrl,'Title',tpl.titleTemplate,Icons.title_rounded),
        const SizedBox(height:10),
        _tf(_fieldCtrl,'Field of study',tpl.field,Icons.school_rounded),
        const SizedBox(height:16),

        // Course grades (if diploma)
        if (tpl.preFilledCourses.isNotEmpty) ...[
          _sectionHeader('Enter grades'),
          ...
          ...tpl.preFilledCourses.asMap().entries.map((e) {
            ...
            return Container(
              ...
              decoration:BoxDecoration(color:_surface,
                ...
                border:Border.all(color:_border)),
              child:Row(children:[
                Container(width:50,padding:const EdgeInsets.symmetric(
                  horizontal:6,vertical:3),
                  decoration:BoxDecoration(color:tpl.bgColor,
                    borderRadius:BorderRadius.circular(6)),
                  child:Text(parts[0],style:GoogleFonts.dmSans(
                    fontSize:10,color:tpl.color,fontWeight:FontWeight.w500))),
                const SizedBox(width:8),
                Expanded(child:Text(parts[1],style:GoogleFonts.dmSans(fontSize:12))),
                const SizedBox(width:8),
                SizedBox(width:72,child:TextField(
                  ...
                  decoration:InputDecoration(hintText:'/20',
                    ...
                    filled:true,fillColor:const Color(0xFFF7F6F2),
        ...
        if (_error != null) ...[
          Container(padding:const EdgeInsets.all(10),
            ...
            child:Text(_error!,style:GoogleFonts.dmSans(color:_red,fontSize:12))),
          const SizedBox(height:10),
        ],

        ElevatedButton.icon(
          icon:const Icon(Icons.verified_rounded,size:18),
          label:const Text('Issue from template'),
  ...
  Widget _doneView() => Center(child:Column(
    ...
    children:[
      ...
      const SizedBox(height:8),
      Text('From template: ${_selected?.name}',
        style:GoogleFonts.dmSans(fontSize:13,color:_textSec)),
      ...
      ElevatedButton(
        ...
        child:const Text('Sign this document')),
      const SizedBox(height:10),
      OutlinedButton(
        ...
        child:const Text('Issue another document')),
    ]));
  ...
  Widget _mentionDropdown() => Column(
    crossAxisAlignment:CrossAxisAlignment.start,
    children:[
      Text('Mention',style:GoogleFonts.dmSans(
      ...
      DropdownButtonFormField<String>(
        ...
        onChanged:(v){if(v!=null)setState(()=>_mentionCtrl.text=v);}),
    ]);
  ...
  Widget _tf(TextEditingController c,String label,String hint,IconData icon,
    ...
    Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text(label,style:GoogleFonts.dmSans(
        fontSize:11,fontWeight:FontWeight.w500,color:_textSec)),
      const SizedBox(height:4),
      TextField(controller:c,readOnly:readOnly,onTap:onTap,
