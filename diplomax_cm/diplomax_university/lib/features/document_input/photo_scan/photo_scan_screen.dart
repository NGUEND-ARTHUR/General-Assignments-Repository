...
class PhotoScanScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<PhotoScanScreen> createState() => _PhotoScanState();
}
...
class _PhotoScanState extends ConsumerState<PhotoScanScreen> {
  ...
  void dispose() {
    _recognizer.close();
    _matCtrl.dispose(); _titleCtrl.dispose(); _typeCtrl.dispose();
    _degreeCtrl.dispose(); _fieldCtrl.dispose(); _mentionCtrl.dispose();
    _dateCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _takePhoto(ImageSource source) async {
    ...
    setState(() { _image = file; _phase = _PhotoPhase.processing; _error = null; });
    ...
    try {
      ...
      _dateCtrl.text    = fields['year'] != null
          ? '${fields['year']}-06-30' : '';

      setState(() => _phase = _PhotoPhase.reviewing);
    } catch (e) {
      setState(() { _phase = _PhotoPhase.idle; _error = 'OCR failed: ${e.toString()}'; });
    }
  }
  ...
  Map<String, String> _parseFields(String text) {
    ...
    // Matricule
    final mat = RegExp(r'(?:ICTU|UY1|UY2|UB)[A-Z0-9]{4,10}', caseSensitive: false)
        .firstMatch(text);
    if (mat != null) result['matricule'] = mat.group(0)!.toUpperCase();
    // Mention
    final mention = RegExp(
        r'(très\s+bien|bien|assez\s+bien|passable)', caseSensitive: false)
        .firstMatch(text);
  ...
  Future<void> _issue() async {
    setState(() { _phase = _PhotoPhase.issuing; _error = null; });
    try {
      ...
      final r = await dio.post('/documents/issue', data: {
        ...
        'document_type':     _typeCtrl.text.trim().isEmpty ? 'certificate' : _typeCtrl.text.trim(),
        'title':             _titleCtrl.text.trim(),
        'degree':            _degreeCtrl.text.trim(),
        'field':             _fieldCtrl.text.trim(),
        'mention':           _mentionCtrl.text.trim().isEmpty ? 'Bien' : _mentionCtrl.text.trim(),
        'issue_date':        _dateCtrl.text.trim().isEmpty ? '2024-06-30' : _dateCtrl.text.trim(),
        'courses':           [],
      });
      setState(() { _issuedDocId = r.data['document_id'] as String?; _phase = _PhotoPhase.done; });
  ...
  Widget _buildBody() {
    switch (_phase) {
      case _PhotoPhase.idle:      return _idle();
      case _PhotoPhase.processing: return _processing();
      case _PhotoPhase.reviewing:  return _reviewing();
      case _PhotoPhase.issuing:
        return const Center(child: CircularProgressIndicator(color: _green));
      case _PhotoPhase.done:      return _done();
    }
  }

  Widget _idle() => Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    // Image preview area
    Container(height:220,width:double.infinity,
      ...
      child:_image!=null
          ...
          : Center(child:Column(mainAxisSize:MainAxisSize.min,children:[
              const Icon(Icons.camera_alt_rounded,color:Colors.white30,size:48),
    ...
    const SizedBox(height:12),
    Text('OCR runs entirely on this device — no internet needed.',
      style:GoogleFonts.dmSans(fontSize:11,color:_textHint)),
  ]);

  Widget _processing() => Center(child:Column(
    ...
    children:[
      ...
      Text('ML Kit OCR is processing the image.',
        style:GoogleFonts.dmSans(fontSize:12,color:_textSec)),
    ]));

  Widget _reviewing() => SingleChildScrollView(child:Column(
    crossAxisAlignment:CrossAxisAlignment.start,
    children:[
      // Thumbnail
      if(_image!=null) Container(height:120,width:double.infinity,
      ...
      Container(padding:const EdgeInsets.all(10),
        ...
        child:Text('Review extracted fields. Correct any errors before issuing.',
          style:GoogleFonts.dmSans(fontSize:11,color:_amber))),
      const SizedBox(height:14),
      _rf('Student matricule *', _matCtrl),
      _rf('Document title *',    _titleCtrl),
      _rf('Document type',       _typeCtrl),
      _rf('Degree',              _degreeCtrl),
      _rf('Field of study',      _fieldCtrl),
      _rf('Mention',             _mentionCtrl),
      _rf('Issue date (YYYY-MM-DD)', _dateCtrl),
      if(_error!=null)...[
        const SizedBox(height:8),
        Container(padding:const EdgeInsets.all(10),
  ...
  Widget _done() => Center(child:Column(
    mainAxisAlignment:MainAxisAlignment.center,
    children:[
      ...
      const SizedBox(height:8),
      Text('From photo scan. Blockchain anchoring in progress.',
  ...
  Widget _rf(String label, TextEditingController ctrl) =>
