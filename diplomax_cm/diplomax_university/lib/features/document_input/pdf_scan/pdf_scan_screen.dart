...
class PdfScanScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<PdfScanScreen> createState() => _PdfScanState();
}
...
class _PdfScanState extends ConsumerState<PdfScanScreen> {
  ...
  @override
  void dispose() {
    _matCtrl.dispose(); _nameCtrl.dispose(); _titleCtrl.dispose();
    _mentionCtrl.dispose(); _univCtrl.dispose(); _yearCtrl.dispose();
    _typeCtrl.dispose(); _degreeCtrl.dispose(); _fieldCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _pickAndUpload() async {
    ...
    try {
      ...
      setState(() { _extracted = extracted; _phase = _Phase.reviewing; });
    } on DioException catch (e) {
      setState(() {
        _error = 'OCR failed: ${(e.response?.data as Map?)?['detail'] ?? e.message}';
        _phase = _Phase.idle;
      });
    }
  }
  ...
  Future<void> _submitReviewed() async {
    setState(() { _phase = _Phase.issuing; _error = null; });
    try {
      final dio = Dio(BaseOptions(baseUrl: const String.fromEnvironment(
          'API_BASE_URL', defaultValue: 'https://api.diplomax.cm/v1')));

      final dateStr = _yearCtrl.text.isNotEmpty
          ? '${_yearCtrl.text}-06-30' : '2024-06-30';
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(
      ...
      leading: BackButton(color: _textPri, onPressed: () => context.go('/issue')),
      title: Text('Scan PDF document',
  ...
  Widget _buildBody() {
    switch (_phase) {
      ...
      case _Phase.uploading: return _buildUploading();
      case _Phase.reviewing: return _buildReview();
      case _Phase.issuing:   return const Center(
  ...
  Widget _buildIdle() => Column(
    ...
    children: [
      ...
      Text(
        ...
        style:GoogleFonts.dmSans(fontSize:13,color:_textSec,height:1.6,
      ...
      if (_error != null) ...[
        Container(padding:const EdgeInsets.all(12),
          ...
          child:Text(_error!,style:GoogleFonts.dmSans(color:Colors.red,fontSize:12))),
  ...
  Widget _buildReview() => SingleChildScrollView(
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(padding:const EdgeInsets.all(12),
        ...
        child:Row(children:[
          ...
          Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
            Text(_fileName??'document.pdf',style:GoogleFonts.dmSans(
              fontSize:12,fontWeight:FontWeight.w500)),
            Text('${(_fileBytes??0)~/1024} KB · ${_extracted.length} fields extracted',
              style:GoogleFonts.dmSans(fontSize:11,color:_textSec)),
          ])),
          TextButton(onPressed:() => setState(()=>_phase=_Phase.idle),
            child:Text('Re-upload',style:GoogleFonts.dmSans(color:_blue,fontSize:12))),
        ])),
      const SizedBox(height:16),
      _infoNote('Review the extracted fields below. Correct any errors before issuing.'),
      const SizedBox(height:16),
      _reviewField('Student matricule *', _matCtrl, Icons.badge_rounded),
      _reviewField('Student full name',   _nameCtrl, Icons.person_rounded),
      _reviewField('Document title *',    _titleCtrl, Icons.title_rounded),
      _reviewField('Document type',       _typeCtrl, Icons.category_rounded),
      _reviewField('Degree',              _degreeCtrl, Icons.school_rounded),
      _reviewField('Field of study',      _fieldCtrl, Icons.science_rounded),
      _reviewField('Mention',             _mentionCtrl, Icons.stars_rounded),
      _reviewField('Year',                _yearCtrl, Icons.calendar_today_rounded),
      if (_error != null) ...[
        const SizedBox(height:8),
      ...
      ElevatedButton.icon(
        ...
        label:const Text('Confirm & issue document'),
  ...
  Widget _buildDone() => Center(child: Column(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      ...
      const SizedBox(height:8),
      Text('From PDF scan. Available in student vault.',
  ...
  Widget _reviewField(String label, TextEditingController ctrl, IconData icon) =>
  ...
  Widget _infoNote(String text) => Container(
    ...
    decoration:BoxDecoration(color:_amberLight,borderRadius:BorderRadius.circular(8),
